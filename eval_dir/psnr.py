...
def get_local_rgb(arr_in, type_):
    if type_ == '1x':
        p = 32
    elif type_ == '2x':
        p = 53
        ...
        p = 68
    return arr_in[p:-p, p:-p, :]

def main():
    f_name = os.listdir(args.contrast)
    if '1x' in args.contrast:
        type_='1x'
    elif '2x' in args.contrast:
        type_='2x'
    else:
        type_='3x'
    res_psnr = []
    res_ssim = []
    for i in tqdm(range(len(f_name))):

        original = np.array(Image.open(args.original + f_name[i]))
        contrast = np.array(Image.open(args.contrast + f_name[i]))
        original = get_local_rgb(original, type_)
        contrast = get_local_rgb(contrast, type_)

        score = psnr_loss(original, contrast)
        if score < 1000:
            res_psnr.append(score)
    
    print(f"Image PSNR Mean: {numpy.mean(res_psnr)}.")
